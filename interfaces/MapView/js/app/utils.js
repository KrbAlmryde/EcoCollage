...
function initNutellaComponents() {
    ...
    subscribeToChannel('projection-layer', adminMessageCallBack);
    // subscribeToChannel("wallscope_channel", adminMessageCallBack);

    // 3. Make asynchronous requests on a certain channel
    nutella.net.request( 'projection-layer', 'start', function(response){
        console.log("Help me 'projection-layer', youre my only hope", response);
    });
}
